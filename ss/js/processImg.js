...
// btn-uploadfile
	window.compressImg = function(inputId,viewId,refWidth,refHeight,callback){
	    ...
	    reader.onload = function( evt ){
	        ...
	        p.onload = function(){
	            ...
	            //压缩换算后的图片高度
			    var afterWidth,afterHeight;
			    
			    if(upImgWidth <= refWidth && upImgHeight <= refHeight) {
					afterWidth = upImgWidth;
					afterHeight = upImgHeight;
				} else {
					if(upImgWidth >= upImgHeight) {
						afterWidth = refWidth;
						afterHeight = parseInt(refWidth * upImgHeight / upImgWidth);
					} else {
						afterHeight = refHeight;
						afterWidth = parseInt(refHeight * upImgWidth / upImgHeight);
					}
				}
